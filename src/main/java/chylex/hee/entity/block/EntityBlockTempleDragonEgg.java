	...
	public void onUpdate(){
		...
		if (field_145812_b >= 24){
			...
			if (!worldObj.isRemote){
				...
				if (deltaY > 6D){
                    List<EntityPlayerMP> players = worldObj.playerEntities.stream().map(player -> (EntityPlayerMP)player).collect(Collectors.toList());
					for(EntityPlayerMP player:new ArrayList<>(players)){
